...
import { removeAccount } from "~/store/auth";

function Account() {
  const { currentAccount, accounts } = useSelector((state) => state.auth);
  const dispatch = useDispatch();
  console.log("current user : ", JSON.stringify(currentAccount, null, 2));

  return (
    <Popover className="relative my-3">
      <PopoverButton className="outline-none flex items-center group w-[100%]">
        <div
          className={
      ...
      >
        {accounts.map((account, index) => (
          <div
            className="flex items-center py-3 px-4 hover:bg-[rgba(231,233,234,0.1)]"
            key={index}
          >
            <img
              src={account.avatar}
            ...
            <div className="mx-3 w-[100%]">
              ...
              <div className="text-[#7e7e7e]">@{account.username}</div>
            </div>
          </div>
        ))}
